...
export const Route = createFileRoute("/_authenticated/expenses")({
  head: () => ({
    meta: [
      { title: "Expenses — Banadir Online FOS" },
      {
        name: "description",
        content: "Business and personal expense ledger with account impact in Banadir Online FOS.",
      },
      { property: "og:title", content: "Expenses — Banadir Online FOS" },
      {
        property: "og:description",
        content: "Every expense posts to an account and rebuilds the daily financial engine.",
      },
...
function ExpensesPage() {
  ...
  const { data, isPending, isError } = useMoneyLedger({ type: "expense", scope, limit: 200 });
    ...
    <div className="space-y-4">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 className="text-xl font-bold sm:text-2xl">Expenses</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Business and personal spending, posted straight into the ledger.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <StatTile label="Total" value={formatMoney(total)} tone="negative" loading={isPending} />
      ...
      >
        {isError ? (
          <p className="py-8 text-center text-sm text-destructive">
            Expenses unavailable — read failed.
          </p>
        ) : isPending ? (
      ...
      <MoneyEntryDialog open={dialogOpen} onOpenChange={setDialogOpen} kind="expense" />
    </div>
  );
}
